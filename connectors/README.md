...
#### `runtime`
Reads a value generated by `famdo` for the current mapping invocation. This
avoids baking sample-specific values into reusable connectors:

```json
{ "type": "runtime", "field": "image-file-name" }
```

Supported fields are:

- `image-file-name`: the basename of the input TIFF;
- `mapping-timestamp`: the UTC timestamp when `famdo map` starts mapping.

These sources are useful for `/General Section/File Name` and
`/General Section/Time Stamp`, whose values describe the generated FAMH output
rather than a TIFF tag.
...
constant; use the `runtime` `image-file-name` source for reusable connectors.

When a method-specific subsection is not yet defined by the FAMH schema, a
connector may create a provisional subsection under `Method Specific` with
descriptive fields. Such a subsection can validate as an extension, but it
must remain marked for human review until its vocabulary is standardized.
